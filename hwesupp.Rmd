---
title: 'Supplement to: Comment on Two Papers about Hardy-Weinberg Equilibrium Tests in Autopolyploids'
author: |
  | David Gerard
...
\begin{abstract}
This supplementary document contains additional simulations, coding examples, and other supporting material for ``Comment on Two Papers about Hardy-Weinberg Equilibrium Tests in Autopolyploids''.
\end{abstract}
...
This document was written in R Markdown and may be explored interactively. All code chunks are executable in the order given.

The package `{hexocto}` contains the code from @wang2021tracing and @wang2022asymptotic, formatted in package form by us so that it is easier to compare. The original repos with the original code are <https://github.com/CCBBeijing/hexaploid> and <https://github.com/CCBBeijing/OctoploidDeer>. You can install this package using `{devtools}`:
...
The package `{hwep}` contains the code from @gerard2022double. We use it for comparison purposes at times. You can install the development version via:
...
We will load these packages into R now:
...
Here, we numerically demonstrate the difference between random mating and equilibrium in autohexaploids.
...
We can verify this by iterating the recursive scheme from @wang2022asymptotic.
```{r}
qw <- q
for (i in 1:20) {
...
We can also verify this by iterating the recursive scheme from @wang2022asymptotic.
```{r}
qw <- q
for (i in 1:20) {
...
We will begin at the same example genotype frequencies as @wang2021tracing.
```{r}
yww <- c(0.1, 0.1, 0.15, 0.1, 0.2, 0.1, 0.05, 0.1, 0.1)
```
Then we apply their recursive approach to obtain their equilibrium genotype frequencies
```{r}
...
```
Our `{hwep}` package, on the other hand, correctly calculates these using our recursive formula
```{r}
qcurrent <- yww
for (i in seq_len(20)) {
...
Their recursive test gets a chi-squared value of 6.602, which we can get here.
```{r}
...
Here, we show that the method @wang2022asymptotic does not produce uniform $p$-values under the null of equilibrium. We also show that our correct version, including the correct degrees of freedom of 5, not 6, does produce uniform $p$-values under the null of equilibrium. We also find the correct degrees of freedom for the recursive test in @wang2021tracing to be 7, not 8.

## Hexaploids

We generate data under the null of equilibrium. We then fit the incorrect method @wang2022asymptotic, our corrected version, and the likelihood ratio test from @gerard2022double.
```{r}
...
We generate data under the incorrect model of @wang2021tracing, calculating the $\chi^2$ statistic each iteration.
```{r}
...
We ran simulations with $\bs{p} = (1, 1, 1, 1)/4$ or $\bs{p} = (0.1, 0.2, 0.3, 0.4)$, $n \in \{100, 200, 400\}$, and $\alpha \in \{0, 1/7, 1/5, 3/11\}$. This mimics the simulation settings from @wang2022asymptotic. We ran each unique combination of parameter settings for 100 replications. Each replication, we generated data according to the assumed model from @wang2022asymptotic, then used their code to obtain estimates of $\bs{p}$ and $\alpha$. We always initialized the algorithm at $\alpha = 0$ and $\bs{p} = (1, 1, 1, 1) / 4$.

Below is our simulation code.
